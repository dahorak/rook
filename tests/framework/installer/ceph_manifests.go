...
func (m *CephManifestsMaster) GetRookOperator(namespace string) string {
	if RookTestOpenshift {
		return getRookOperatorForOpenshiftYaml(namespace, m)
	}
	return getRookOperatorYaml(namespace, m)
}

func getRookOperatorYaml(namespace string, m *CephManifestsMaster) string {
...
// GetRookOperatorForOpenshift returns rook Operator manifest for openshift
func getRookOperatorForOpenshiftYaml(namespace string, m *CephManifestsMaster) string {
	return `---
...
spec:
  ...
  storage:
    useAllNodes: true
    useAllDevices: ` + strconv.FormatBool(settings.UseAllDevices) + kubernetesClusterSettings
}
