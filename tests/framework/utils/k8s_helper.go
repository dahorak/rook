...
var (
	...
	RetryLoop         = getRetryLoopNumber()
	RookTestOpenshift = os.Getenv("ROOK_TEST_OPENSHIFT") == "true"
)

func getRetryLoopNumber() int {
	// For openshift execution we need to increase retry loop as deployment
	// takes more time
	if RookTestOpenshift {
		return 60
	}
...
func (k8sh *K8sHelper) Kubectl(args ...string) (string, error) {
	result, err := k8sh.executor.ExecuteCommandWithTimeout(false, 15*time.Second, kubectlCmd, kubectlCmd, args...)
	if err != nil {
...
func getKubeConfig(executor exec.Executor) (*rest.Config, error) {
	context, err := executor.ExecuteCommandWithOutput(false, "", kubectlCmd, "config", "view", "-o", "json")
	if err != nil {
		k8slogger.Errorf("Errors Encountered while executing %s command : %v", kubectlCmd, err)
	}
